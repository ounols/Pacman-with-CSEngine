...
void main(void) {
	...
	lowp float metallic  = u_metallic >= 0 ? u_metallic : texture(u_sampler_metallic, v_textureCoordOut).r;
	lowp float roughness = u_roughness >= 0 ? u_roughness : texture(u_sampler_roughness, v_textureCoordOut).r;
	lowp float ao        = 1.f;
